...
/**
 * read_line - reads a line from a file descriptor and returns the number
...
int read_line(char **line, int fd)
{
	...
	int i, line_size = 0, line_capacity = BUFFER_SIZE, old_line_capacity;
	...
	*line = malloc(line_capacity); /* allocate memory for a new line */
	...
	while (1)
	{
		...
		for (i = buff_pos; i < buff_len; i++, line_size++) /*res last buff_pos*/
		{ /* buffer has content, so copy from buffer into line */
			if (line_size == line_capacity) /*realloc line if capacity reached*/
			{
				old_line_capacity = line_capacity;
				line_capacity *= LINE_FACTOR; /*increase capacity by a factor*/
				*line = _realloc(*line, old_line_capacity, line_capacity);
				if (*line == NULL) /* alloc fail */
					return (-1);
			}
